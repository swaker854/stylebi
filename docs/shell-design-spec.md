...
Secondary and default controls should feel neutral, not like colored brand buttons. They should step down from the primary action by fill weight (accent fill → subtle neutral fill → transparent), not by adding a resting border.
...
The shell uses three emphasis tiers plus two modifiers. The neutral tiers step down by fill weight, not by border.

- Primary (tier 1): filled accent button — the one headline action
- Secondary (tier 2): filled neutral button — subtle surface fill, no resting border
- Ghost (tier 3): transparent button that fills on hover; the dominant style by count
- Danger (modifier): a destructive flavor applied to Primary (filled red) or Ghost (red text), not a standalone neutral button
- Icon (modifier): a square Ghost for toolbar and row actions
...
- most shell actions should usually be ghost or icon controls
...
| Radius | `3px` |
| Font | `12px`, `500` |
| Border | `1px` on Primary only; neutral tiers are borderless (fill / transparent) |
...
- `Secondary` (`neutral fill`)
  ...
  - common candidates: add, connect, refresh, move, clear, chooser/open-browser actions — but only when one such action clearly stands above its neighbours; otherwise keep them as Ghost
- `Ghost` (`transparent`)
...
- `Danger` (`modifier`) — a destructive flavor layered on Primary or Ghost, not a separate neutral button
...
- Primary buttons use shell primary (filled accent).
- Secondary buttons use a subtle neutral fill with no resting border, and darken the fill on hover.
- Ghost buttons are transparent at rest and fill on hover; they are the dominant button style by count on most screens.
- Danger is a modifier: a filled-red CTA with white text for destructive confirmation, or a red-text Ghost for inline/row destructive actions. Raise contrast on hover — never leave dark text on a saturated red fill.
- Icon buttons are square Ghost buttons — borderless until hover or focus.
...
| Secondary button | neutral soft fill + text-default, no resting border |
| Ghost button | transparent + surface-hover on hover |
| Icon button | square ghost + surface-hover on hover |
| Danger button | danger family (filled CTA or red-text ghost) |

...
- the shell button model is **three emphasis tiers + two modifiers**: Primary (filled accent), Secondary (subtle neutral fill, no border), Ghost (`.btn-default`, transparent + hover fill), plus Icon (square Ghost) and Danger (modifier on Primary/Ghost)
- `.btn-light` / `.btn-light-no-bg` are a duplicate third neutral and should be folded into Secondary / Ghost and retired

Implementation summary:

- remap primary, secondary, and default button tokens in `_variables.scss` (Secondary → subtle neutral fill with no border, Default/Ghost → transparent), add a `--inet-button-danger-*` family, retire `--inet-button-light-*`, then finish shared button sizing, radius, padding, typography, and a real `.btn-icon` spec in [_bootstrap-override.scss](E:\home\dev\github\stylebi-visual_BI_tool\stylebi\web\projects\portal\src\scss\_bootstrap-override.scss:1)
...
| Token | Action | Before | After | Notes |
|---|---|---|---|---|
| `--inet-button-secondary-bg-color` | update | `var(--inet-secondary-color)` | `var(--inet-shell-surface-subtle)` | subtle neutral fill (tier 2) |
| `--inet-button-secondary-text-color` | update | `white` | `var(--inet-text-color)` | readable neutral text |
| `--inet-button-secondary-border-color` | update | `var(--inet-secondary-color)` | `transparent` | no resting border |
...
| `--inet-button-secondary-hover-text-color` | update | `white` | `var(--inet-text-color)` | readable hover text |
| `--inet-button-secondary-hover-border-color` | update | `var(--inet-secondary-color-dark)` | `transparent` | stays borderless on hover |
...
| `--inet-button-default-text-color` | update | `var(--inet-secondary-color)` | `var(--inet-text-color)` | Ghost text/icon |
| `--inet-button-default-border-color` | update | `var(--inet-secondary-color)` | `transparent` | no persistent outline (selector must honor this token) |
| `--inet-button-default-hover-bg-color` | update | `var(--inet-secondary-color)` | `var(--inet-ui-neutral-hover-bg-color)` | subtle hover fill |
| `--inet-button-default-hover-text-color` | update | `var(--inet-secondary-color-light)` | `var(--inet-text-color)` | Ghost hover text |
...
| `--inet-button-secondary-bg-color` | update | `var(--inet-secondary-color)` | `var(--inet-shell-surface-subtle)` | subtle neutral fill, no resting border (Figma-style tier 2) |
| `--inet-button-secondary-text-color` | update | `white` | `var(--inet-text-color)` | readable neutral text |
| `--inet-button-secondary-border-color` | update | `var(--inet-secondary-color)` | `transparent` | no resting border — tier reads by fill |
...
| `--inet-button-default-text-color` | update | `var(--inet-secondary-color)` | `var(--inet-text-color)` | Ghost text color |
...
| `--inet-button-danger-bg-color` | add | missing | `var(--inet-danger-color)` | filled-red destructive CTA |
| `--inet-button-danger-text-color` | add | missing | `white` | high-contrast label |
| `--inet-button-danger-border-color` | add | missing | `var(--inet-danger-color)` | matches fill |
| `--inet-button-danger-hover-bg-color` | add | missing | `#B04444` | darkened danger hover step |
| `--inet-button-danger-hover-text-color` | add | missing | `white` | stays high-contrast |
| `--inet-button-danger-hover-border-color` | add | missing | `#B04444` | matches hover fill |
...
| `.btn.btn-default` | `border` | update | hardcodes `1px solid var(--inet-default-border-color)` at rest | `1px solid var(--inet-button-default-border-color)` | **fix**: honor the token so the Ghost tier is truly borderless at rest |
| `.btn.btn-danger` | color/background/border bindings | update | semantic tokens direct (neutral rest border, low-contrast hover) | bind to the new `--inet-button-danger-*` family | filled-red CTA, white text, darkened hover |
| `.btn.btn-light, .btn.btn-light-no-bg` | whole rule | remove | own `--inet-button-light-*` family | fold callers into `.btn-secondary` (raised) / `.btn-default` (transparent) | retire the duplicate third neutral |
| `.btn.btn-icon` | size / centering / hover | add | only `flex: 0 1 4em` inside input-groups | square `var(--inet-control-height-md)` box, centered glyph, transparent until `:hover`/`:focus` | give icon buttons a real visual contract (Ghost behavior) |
